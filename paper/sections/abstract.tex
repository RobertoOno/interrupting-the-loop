...
over habituation alone (paired permutation $p=0.002$), matches the full
scaffold on surprise and beats it on connection. Controls show what carries
the effect: a bare paragraph break does nothing and a continuity connective
hurts; the new subject carries most of it and works on a reset context at
least as well as on a preserved one; injecting the premise or the stream's
own past is as bad as not interrupting; salience timing is no better than a
clock. The gains are local: judged as whole documents, no arm builds an
integrated text. \todo{the confirmatory replication} The ordering replicates
