...
sees the computation. We re-run every finished stream through its generator
...
condition's at every layer; its explored radius is 0.13 at the input layers
...
commitment. Bare generation freezes at every layer, most at the surface.}
\label{fig:n1}
\end{figure}
...
correlates with surprise at the input layers ($\rho=+0.47$ at layer 0) and
not at the top ($-0.04$), but the pooled number is carried by the
...
($\rho$ +0.3 to +0.6): confident is unsurprising. What the judge calls
surprise is surface departure with deep continuity: new at the surface,
...
with depth: forgetting is a deep re-encounter with the beginning, in the
network's own representation. Across a clock reseed \emph{over preserved
context} the same measures are +0.01--0.03 and $\approx$0 at every
layer, and this is the interruption the judge rewards most. Judged surprise
and connection are not deep representational shifts; they are surface
departures over an intact deep context, which is also why this arm keeps the
most connection. The dissociation replicates on the 8B (+0.03 to +0.10 vs
...
the scaffold is a top-layer phenomenon.
