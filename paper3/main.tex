...
decomposes additively, memory carrying the largest share, and anchoring
survives as collapse prevention: memory+repulsion is the only arm that
...
in hundreds, and its gains arrive without the operators; the tested grid,
verifier-time and budget changes move nothing. The search stalls after
...
We state as a registered hypothesis that the remaining gap is a matter of
construction family, an idea, and release the harness, every candidate,
and the dated pre-registrations.
\end{abstract}
...
The mean is $+0.196$; the median, $+0.045$; leave-one-problem-out means
stay between $+0.153$ and $+0.224$.}
\label{fig:forest}
\end{figure}
...
Family hint & registered & all above & optimize-once-given & to run \\
\bottomrule
\end{tabular}
