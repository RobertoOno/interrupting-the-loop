...
gap ($+0.196$ \ci{+0.059}{+0.343}, nominal $p=0.023$; median
per-problem effect $+0.045$; robust to removing the seed floor from the
metric). Repulsion raises construction-hash diversity everywhere
...
hundreds, and its gains arrive without the operators. The search stalls
...
and a characterized wall: the tested grid and verifier-time changes did
not improve the result, a stronger proposer raised and accelerated the
plateau without closing the final gap, and the construction family
remains the leading suspect for the binding constraint. (Reference
percentages are raw value ratios; frac in the text is gap-based.)
...
we measured them: the certified gains are a local-proposer phenomenon,
...
it (no oracle run deserted to the better sparse family). Family
membership is decided by an explicit rule (at most 50 support atoms,
mass at least $0.3$ on $\{0..3\}$, atoms in at least three
$\log_{10}$ distance-from-top bands; per-run table released): by it,
eight of nine hint-family finals are in-family and zero of six
no-hint/generic finals are. No unaided candidate in the three registered
120-sample runs was classified in-family. Two caveats bound the reading:
with three seeds per condition these contrasts are descriptive (the
smallest one-sided sign-flip $p$ at $n{=}3$ is $0.125$), and the
historical $0.3816$ comes from a different battery under strong seed
variance, so the oracle's $0.375$ exceeds the \emph{contemporary}
no-hint $0.352$ descriptively while trailing the historical best. Within
those bounds, the registered hypothesis was not supported: handing the
idea, in words or in code, did not cross the last mile under this
budget, and our best implementation of the idea was not the missing step
at $L{=}5000$. What the experiment does establish is a
