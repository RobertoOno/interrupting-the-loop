...
# LAUNCH:   nohup sh scripts/run_f_ext2.sh > runs/frontier/F_ext2_chain.log 2>&1 &
cd "$(dirname "$0")/.." || exit 1
...
echo "F-EXT2 START $(date)"
for rep in "" "_s1"; do
  seed=0; [ "$rep" = "_s1" ] && seed=1
  for prob in maxmin16 heiltri11 factn180; do
...
$PY scripts/frontier_analysis.py > $R/analysis_ext2.log 2>&1
echo "F-EXT2 DONE $(date)"
