...
gen() { # arm cycle adapter variants seed
  caffeinate -is $PY scripts/consolidate.py gen --arm $1 --cycle $2 --adapter $3 --variants $4 --n $NB --tokens $TOK --seed $5 --out $ROOT/$1_c$2 2>&1 | grep -v Warning
...
while [ $c -le $CY ]; do
  ...
  gen attract $c $ROOT/adapter_attract_$c train $c
  gen attract $c $ROOT/adapter_attract_$c heldout $c
  gen random $c $ROOT/adapter_random_$c train $c
  gen random $c $ROOT/adapter_random_$c heldout $c
