# Rater guide — blind rating of machine-written passages (pack v2)

Thank you for helping with a research study on text generation. The task takes
about 60–90 minutes and needs only a web browser.

## What you will see

The file `pack_v2.html` contains **63 items**. Each item shows:

- **Earlier text** (grey box): the last ~1,500 characters written before the
  passage. Read it first — the first score depends on it.
- **The passage** (blue box): about 100–120 words. This is what you rate.
...
- Passages are cut at fixed positions, so a passage may begin or end
  mid-sentence; do not penalize that.

Score the two dimensions **independently**: a passage can be surprising and
incoherent, or coherent and dull. **Use the whole scale** — many passages
deserve 0–2 on surprise; a few deserve 7+.

## Practical

1. Open `pack_v2.html` in any browser (Chrome, Safari, Firefox).
...
about" we will answer gladly *after* you send your scores.
