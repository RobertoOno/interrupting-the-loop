# Appendix — battery 2 (auto-generated by scripts/analysis_b2.py)

1063 judged windows total (801 from battery 2).


### Q1 — the confound: habituation without interruption

| condition | windows | kind | surprise | connection | coherence | good (S≥5&H≥5) |
|---|---|---|---|---|---|---|
| bare (no habituation, no interruption) | 50 | jump | 0.28 ± 0.72 | 0.20 ± 0.57 | 2.06 ± 2.01 | 0/50 |
| bare + habituation (no interruption) | 60 | clock | 1.70 ± 1.71 | 1.08 ± 1.10 | 4.08 ± 2.19 | 4/60 |
| bare + habituation + clock reseed 150 | 60 | clock | 3.08 ± 1.48 | 3.15 ± 1.53 | 4.78 ± 0.95 | 12/60 |
| DREAM scaffold (λ=0) | 47 | crystallize/jump/recurrence | 3.19 ± 1.92 | 1.64 ± 1.56 | 3.68 ± 1.69 | 8/47 |

vs **bare + habituation (no interruption)** (Δ mean, 95% bootstrap CI over windows; Cliff's δ; Mann-Whitney p; paired-by-seed Δ [CI]):

| condition | dim | Δ [CI] | δ | p | paired-by-seed Δ [CI] (n seeds) |
|---|---|---|---|---|---|
| bare (no habituation, no interruption) | surprise | **-1.42 [-1.89, -0.95]** | -0.58 | 1.4e-08 | -1.42 [-1.72, -1.12] (10) |
| bare (no habituation, no interruption) | connection | **-0.88 [-1.20, -0.56]** | -0.52 | 1.1e-07 | -0.88 [-1.22, -0.54] (10) |
| bare (no habituation, no interruption) | coherence | **-2.02 [-2.79, -1.23]** | -0.55 | 3.4e-07 | -2.02 [-3.37, -0.68] (10) |
| bare + habituation + clock reseed 150 | surprise | **+1.38 [+0.80, +1.93]** | +0.50 | 1.3e-06 | +1.38 [+0.92, +1.92] (10) |
| bare + habituation + clock reseed 150 | connection | **+2.07 [+1.60, +2.55]** | +0.75 | 5.2e-13 | +2.07 [+1.63, +2.57] (10) |
| bare + habituation + clock reseed 150 | coherence | **+0.70 [+0.10, +1.30]** | +0.21 | 4.3e-02 | +0.70 [-0.27, +1.72] (10) |
| DREAM scaffold (λ=0) | surprise | **+1.49 [+0.80, +2.19]** | +0.46 | 3.4e-05 | +1.80 [+0.97, +2.75] (10) |
| DREAM scaffold (λ=0) | connection | **+0.55 [+0.06, +1.10]** | +0.20 | 6.2e-02 | +0.61 [+0.22, +1.01] (10) |
| DREAM scaffold (λ=0) | coherence | -0.40 [-1.14, +0.34] | -0.10 | 3.8e-01 | -0.26 [-1.03, +0.56] (10) |

### Q2 — content of the interruption (windows right before each injection)

| condition | windows | kind | surprise | connection | coherence | good (S≥5&H≥5) |
|---|---|---|---|---|---|---|
| neutral subject change (clock 150) | 60 | clock | 3.08 ± 1.48 | 3.15 ± 1.53 | 4.78 ± 0.95 | 12/60 |
| re-encounter stitch (clock 150) | 50 | cut | 2.68 ± 1.46 | 3.64 ± 1.57 | 4.72 ± 1.33 | 5/50 |
| the premise itself (clock 150) | 50 | cut | 1.14 ± 1.73 | 1.04 ± 1.57 | 3.18 ± 1.90 | 3/50 |
| the stream's own past (clock 150) | 50 | cut | 1.42 ± 1.43 | 1.52 ± 1.35 | 3.06 ± 1.42 | 2/50 |

vs **neutral subject change (clock 150)** (Δ mean, 95% bootstrap CI over windows; Cliff's δ; Mann-Whitney p; paired-by-seed Δ [CI]):

| condition | dim | Δ [CI] | δ | p | paired-by-seed Δ [CI] (n seeds) |
|---|---|---|---|---|---|
| re-encounter stitch (clock 150) | surprise | -0.40 [-0.95, +0.14] | -0.13 | 2.4e-01 | -0.40 [-1.29, +0.46] (10) |
| re-encounter stitch (clock 150) | connection | +0.49 [-0.09, +1.07] | +0.20 | 7.1e-02 | +0.49 [-0.56, +1.56] (10) |
| re-encounter stitch (clock 150) | coherence | -0.06 [-0.50, +0.38] | -0.00 | 9.8e-01 | -0.06 [-0.75, +0.57] (10) |
| the premise itself (clock 150) | surprise | **-1.94 [-2.53, -1.31]** | -0.68 | 6.1e-10 | -1.94 [-2.65, -1.30] (10) |
| the premise itself (clock 150) | connection | **-2.11 [-2.68, -1.51]** | -0.72 | 4.1e-11 | -2.11 [-2.83, -1.46] (10) |
| the premise itself (clock 150) | coherence | **-1.60 [-2.15, -1.01]** | -0.53 | 1.2e-06 | -1.60 [-2.36, -0.70] (10) |
| the stream's own past (clock 150) | surprise | **-1.66 [-2.21, -1.11]** | -0.61 | 2.1e-08 | -1.66 [-2.26, -1.07] (10) |
| the stream's own past (clock 150) | connection | **-1.63 [-2.17, -1.08]** | -0.62 | 9.0e-09 | -1.63 [-2.37, -0.91] (10) |
| the stream's own past (clock 150) | coherence | **-1.72 [-2.18, -1.24]** | -0.70 | 1.1e-10 | -1.72 [-2.30, -1.11] (10) |

### Q3 — timing of the re-encounter: clock vs salience

| condition | windows | kind | surprise | connection | coherence | good (S≥5&H≥5) |
|---|---|---|---|---|---|---|
| re-encounter on the clock (150) | 50 | cut | 2.68 ± 1.46 | 3.64 ± 1.57 | 4.72 ± 1.33 | 5/50 |
| re-encounter on the clock (900; matched frequency) | 12 | cut | 2.83 ± 1.91 | 2.25 ± 1.74 | 3.92 ± 1.44 | 2/12 |
| neutral change on the clock (900; matched frequency) | 16 | cut | 1.88 ± 1.32 | 1.69 ± 1.10 | 5.75 ± 2.02 | 0/16 |
| re-encounter on salience events (event windows) | 80 | crystallize/cut/jump/recurrence | 3.92 ± 1.42 | 2.99 ± 1.73 | 3.75 ± 1.18 | 8/80 |
| re-encounter on salience events (uniform windows) | 60 | clock | 2.18 ± 2.04 | 1.68 ± 1.65 | 3.93 ± 1.88 | 4/60 |
| re-encounter on the clock (900), uniform windows | 18 | clock | 4.39 ± 1.60 | 3.50 ± 1.54 | 5.00 ± 1.63 | 7/18 |
| neutral change on the clock (900), uniform windows | 22 | clock | 5.45 ± 2.08 | 2.91 ± 2.00 | 5.95 ± 1.26 | 16/22 |
| salience only, no injection (event windows) | 38 | crystallize/jump/recurrence | 2.63 ± 1.75 | 1.39 ± 1.42 | 3.71 ± 1.60 | 2/38 |

vs **re-encounter on the clock (150)** (Δ mean, 95% bootstrap CI over windows; Cliff's δ; Mann-Whitney p; paired-by-seed Δ [CI]):

| condition | dim | Δ [CI] | δ | p | paired-by-seed Δ [CI] (n seeds) |
|---|---|---|---|---|---|
| re-encounter on the clock (900; matched frequency) | surprise | +0.15 [-0.97, +1.31] | +0.04 | 8.6e-01 | +0.50 [-0.55, +1.75] (3) |
| re-encounter on the clock (900; matched frequency) | connection | **-1.39 [-2.41, -0.25]** | -0.46 | 1.2e-02 | -1.35 [-3.60, +1.00] (3) |
| re-encounter on the clock (900; matched frequency) | coherence | -0.80 [-1.72, +0.08] | -0.29 | 1.1e-01 | -0.62 [-2.25, +1.55] (3) |
| neutral change on the clock (900; matched frequency) | surprise | **-0.81 [-1.56, -0.02]** | -0.30 | 6.5e-02 | -0.68 [-1.75, +0.40] (4) |
| neutral change on the clock (900; matched frequency) | connection | **-1.95 [-2.65, -1.27]** | -0.65 | 7.4e-05 | -1.81 [-3.07, -0.64] (4) |
| neutral change on the clock (900; matched frequency) | coherence | +1.03 [-0.06, +2.06] | +0.36 | 3.0e-02 | +1.30 [+0.02, +2.57] (4) |
| re-encounter on salience events (event windows) | surprise | **+1.24 [+0.73, +1.75]** | +0.44 | 2.0e-05 | +1.10 [+0.34, +1.85] (10) |
| re-encounter on salience events (event windows) | connection | **-0.65 [-1.23, -0.06]** | -0.24 | 2.2e-02 | -0.92 [-1.55, -0.29] (10) |
| re-encounter on salience events (event windows) | coherence | **-0.97 [-1.41, -0.52]** | -0.39 | 1.5e-04 | -0.86 [-1.72, -0.06] (10) |
| re-encounter on salience events (uniform windows) | surprise | -0.50 [-1.14, +0.15] | -0.21 | 5.5e-02 | -0.50 [-1.11, -0.01] (10) |
| re-encounter on salience events (uniform windows) | connection | **-1.96 [-2.54, -1.35]** | -0.61 | 2.7e-08 | -1.96 [-2.62, -1.37] (10) |
| re-encounter on salience events (uniform windows) | coherence | **-0.79 [-1.38, -0.19]** | -0.25 | 2.1e-02 | -0.79 [-1.31, -0.29] (10) |
| re-encounter on the clock (900), uniform windows | surprise | **+1.71 [+0.81, +2.51]** | +0.58 | 2.4e-04 | +2.06 [+1.20, +2.67] (3) |
| re-encounter on the clock (900), uniform windows | connection | -0.14 [-0.98, +0.68] | -0.02 | 9.3e-01 | -0.10 [-1.43, +1.33] (3) |
| re-encounter on the clock (900), uniform windows | coherence | +0.28 [-0.55, +1.11] | +0.09 | 5.6e-01 | +0.47 [-0.07, +0.97] (3) |
| neutral change on the clock (900), uniform windows | surprise | **+2.77 [+1.77, +3.68]** | +0.69 | 2.7e-06 | +2.85 [+1.46, +4.23] (4) |
| neutral change on the clock (900), uniform windows | connection | -0.73 [-1.67, +0.22] | -0.25 | 9.2e-02 | -0.62 [-2.92, +1.57] (4) |
| neutral change on the clock (900), uniform windows | coherence | **+1.23 [+0.56, +1.84]** | +0.53 | 2.5e-04 | +1.55 [+0.20, +2.38] (4) |
| salience only, no injection (event windows) | surprise | -0.05 [-0.73, +0.64] | -0.05 | 7.0e-01 | +0.18 [-0.80, +1.31] (10) |
| salience only, no injection (event windows) | connection | **-2.25 [-2.87, -1.62]** | -0.73 | 3.8e-09 | -2.11 [-3.00, -1.16] (10) |
| salience only, no injection (event windows) | coherence | **-1.01 [-1.64, -0.37]** | -0.35 | 4.5e-03 | -0.87 [-1.92, +0.12] (10) |

### Q4 — frequency of interruption (neutral seed)

| condition | windows | kind | surprise | connection | coherence | good (S≥5&H≥5) |
|---|---|---|---|---|---|---|
| every 75 (before injection) | 50 | cut | 0.86 ± 1.04 | 1.28 ± 1.04 | 3.08 ± 0.63 | 1/50 |
| every 150 (before injection) | 60 | clock | 3.08 ± 1.48 | 3.15 ± 1.53 | 4.78 ± 0.95 | 12/60 |
| every 300 (before injection) | 50 | cut | 2.90 ± 1.50 | 2.70 ± 1.63 | 5.88 ± 0.82 | 7/50 |
| every 600 (before injection) | 50 | cut | 3.06 ± 1.58 | 2.56 ± 1.66 | 5.32 ± 1.62 | 6/50 |
| every 900 (before injection) | 16 | cut | 1.88 ± 1.32 | 1.69 ± 1.10 | 5.75 ± 2.02 | 0/16 |
| every 300 (mid-segment) | 60 | clock | 4.93 ± 1.53 | 3.47 ± 1.68 | 6.07 ± 0.98 | 34/60 |
| every 600 (mid-segment) | 60 | clock | 4.12 ± 1.83 | 2.70 ± 1.63 | 5.65 ± 1.54 | 23/60 |
| every 900 (mid-segment) | 22 | clock | 5.45 ± 2.08 | 2.91 ± 2.00 | 5.95 ± 1.26 | 16/22 |

vs **every 150 (before injection)** (Δ mean, 95% bootstrap CI over windows; Cliff's δ; Mann-Whitney p; paired-by-seed Δ [CI]):

| condition | dim | Δ [CI] | δ | p | paired-by-seed Δ [CI] (n seeds) |
|---|---|---|---|---|---|
| every 75 (before injection) | surprise | **-2.22 [-2.69, -1.75]** | -0.83 | 1.7e-14 | -2.22 [-2.91, -1.73] (10) |
| every 75 (before injection) | connection | **-1.87 [-2.35, -1.38]** | -0.76 | 1.1e-12 | -1.87 [-2.49, -1.32] (10) |
| every 75 (before injection) | coherence | **-1.70 [-2.00, -1.41]** | -0.86 | 1.5e-15 | -1.70 [-2.00, -1.39] (10) |
| every 300 (before injection) | surprise | -0.18 [-0.74, +0.37] | -0.06 | 5.7e-01 | -0.18 [-0.81, +0.39] (10) |
| every 300 (before injection) | connection | -0.45 [-1.04, +0.12] | -0.18 | 1.1e-01 | -0.45 [-1.31, +0.45] (10) |
| every 300 (before injection) | coherence | **+1.10 [+0.78, +1.42]** | +0.59 | 2.5e-08 | +1.10 [+0.69, +1.57] (10) |
| every 600 (before injection) | surprise | -0.02 [-0.60, +0.54] | -0.00 | 9.8e-01 | -0.02 [-0.74, +0.60] (10) |
| every 600 (before injection) | connection | -0.59 [-1.19, +0.01] | -0.21 | 4.9e-02 | -0.59 [-1.28, +0.03] (10) |
| every 600 (before injection) | coherence | **+0.54 [+0.03, +1.03]** | +0.26 | 1.8e-02 | +0.54 [-0.09, +1.13] (10) |
| every 900 (before injection) | surprise | **-1.21 [-1.94, -0.45]** | -0.45 | 5.3e-03 | -0.96 [-2.06, -0.21] (4) |
| every 900 (before injection) | connection | **-1.46 [-2.12, -0.80]** | -0.53 | 9.8e-04 | -1.27 [-2.88, +0.08] (4) |
| every 900 (before injection) | coherence | +0.97 [-0.09, +1.96] | +0.38 | 1.8e-02 | +0.79 [-1.40, +2.33] (4) |
| every 300 (mid-segment) | surprise | **+1.85 [+1.30, +2.38]** | +0.60 | 9.0e-09 | +1.85 [+1.23, +2.33] (10) |
| every 300 (mid-segment) | connection | +0.32 [-0.27, +0.88] | +0.13 | 2.0e-01 | +0.32 [-0.30, +0.77] (10) |
| every 300 (mid-segment) | coherence | **+1.28 [+0.93, +1.62]** | +0.63 | 1.1e-09 | +1.28 [+0.88, +1.68] (10) |
| every 600 (mid-segment) | surprise | **+1.03 [+0.45, +1.62]** | +0.33 | 1.3e-03 | +1.03 [+0.58, +1.47] (10) |
| every 600 (mid-segment) | connection | -0.45 [-1.03, +0.10] | -0.17 | 9.1e-02 | -0.45 [-1.00, +0.17] (10) |
| every 600 (mid-segment) | coherence | **+0.87 [+0.40, +1.32]** | +0.42 | 4.6e-05 | +0.87 [+0.32, +1.43] (10) |
| every 900 (mid-segment) | surprise | **+2.37 [+1.40, +3.27]** | +0.64 | 6.9e-06 | +2.56 [+2.21, +2.94] (4) |
| every 900 (mid-segment) | connection | -0.24 [-1.12, +0.69] | -0.13 | 3.7e-01 | -0.08 [-0.17, +0.00] (4) |
| every 900 (mid-segment) | coherence | **+1.17 [+0.56, +1.72]** | +0.58 | 3.3e-05 | +1.04 [+0.42, +1.71] (4) |

### Q4b — phase within the segment: score by tokens since the last interruption (all judged windows, both kinds)

| every | windows | pooled surprise | conn | coh | early (≤160 tokens since injection) S / C / H | late (>160) S / C / H |
|---|---|---|---|---|---|---|
| 75 | 60 | 0.88 | 1.35 | 3.15 | 0.88 / 1.35 / 3.15 (n=60) | — / — / — (n=0) |
| 150 | 60 | 3.08 | 3.15 | 4.78 | 3.08 / 3.15 / 4.78 (n=60) | — / — / — (n=0) |
| 300 | 110 | 4.01 | 3.12 | 5.98 | 5.28 / 3.35 / 5.95 (n=40) | 3.29 / 2.99 / 6.00 (n=70) |
| 600 | 110 | 3.64 | 2.64 | 5.50 | 5.40 / 3.20 / 5.65 (n=20) | 3.24 / 2.51 / 5.47 (n=90) |
| 900 | 38 | 3.95 | 2.39 | 5.87 | 6.00 / 2.87 / 6.20 (n=15) | 2.61 / 2.09 / 5.65 (n=23) |

### Q5 — second generator family (Qwen3-8B-Base, 10 seeds)

| condition | windows | kind | surprise | connection | coherence | good (S≥5&H≥5) |
|---|---|---|---|---|---|---|
| 8B: bare | 33 | clock/jump | 0.88 ± 1.34 | 0.55 ± 0.86 | 3.09 ± 1.98 | 0/33 |
| 8B: bare + habituation | 18 | clock | 1.78 ± 1.55 | 0.67 ± 0.67 | 4.89 ± 2.60 | 2/18 |
| 8B: bare + habituation + clock reseed 150 | 13 | clock/cut | 3.73 ± 1.62 | 4.23 ± 1.58 | 3.92 ± 0.83 | 0/13 |
| 8B: DREAM scaffold (λ=0) | 31 | clock/crystallize/cut/jump | 1.74 ± 1.66 | 0.58 ± 0.66 | 3.81 ± 1.96 | 0/31 |

vs **8B: bare + habituation** (Δ mean, 95% bootstrap CI over windows; Cliff's δ; Mann-Whitney p; paired-by-seed Δ [CI]):

| condition | dim | Δ [CI] | δ | p | paired-by-seed Δ [CI] (n seeds) |
|---|---|---|---|---|---|
| 8B: bare | surprise | **-0.90 [-1.76, -0.09]** | -0.38 | 1.7e-02 | -0.90 [-1.80, -0.03] (3) |
| 8B: bare | connection | -0.12 [-0.54, +0.29] | -0.15 | 3.1e-01 | -0.12 [-0.50, +0.29] (3) |
| 8B: bare | coherence | **-1.80 [-3.14, -0.42]** | -0.36 | 3.1e-02 | -1.80 [-5.64, +1.61] (3) |
| 8B: bare + habituation + clock reseed 150 | surprise | **+1.95 [+0.76, +3.07]** | +0.59 | 5.1e-03 | +1.75 [+0.17, +4.25] (3) |
| 8B: bare + habituation + clock reseed 150 | connection | **+3.56 [+2.62, +4.43]** | +0.94 | 7.1e-06 | +3.22 [+2.50, +4.67] (3) |
| 8B: bare + habituation + clock reseed 150 | coherence | -0.97 [-2.22, +0.35] | -0.38 | 7.4e-02 | -0.39 [-2.33, +2.17] (3) |
| 8B: DREAM scaffold (λ=0) | surprise | -0.04 [-0.99, +0.87] | -0.04 | 8.0e-01 | — |
| 8B: DREAM scaffold (λ=0) | connection | -0.09 [-0.47, +0.29] | -0.07 | 6.5e-01 | — |
| 8B: DREAM scaffold (λ=0) | coherence | -1.08 [-2.44, +0.29] | -0.24 | 1.6e-01 | — |
